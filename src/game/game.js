  ...
  start(cfgs, arenaOpts = {}) {
    ...
    for (const c of this.circles) {
      ...
      if (tm) {
        ...
        if (tm.contactSlow)  { c._contactSlowFactor = tm.contactSlow; c._contactSlowDur = tm.slowDuration ?? 3; }
    ...
    if (vRel > 0) {
      ...
      if (!sameTeam && vRel > 100) {
        ...
        if (c1Hit && c2Hit) {
          ...
          if ((c1._contactSlowFactor ?? 0) > 0) c2.applyContactSlow(c1._contactSlowFactor, c1._contactSlowDur ?? 3);
          if ((c2._contactBleedDPS   ?? 0) > 0) c1.applyBleed(c2._contactBleedDur ?? 3, c2._contactBleedDPS);
          if ((c2._contactSlowFactor ?? 0) > 0) c1.applyContactSlow(c2._contactSlowFactor, c2._contactSlowDur ?? 3);
